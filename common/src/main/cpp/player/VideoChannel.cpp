...
#include "../pusher/include/VideoChannel.h"
...
//             video_time);
        ...
        if (av_time_diff > 0) {
           ...
           // LOGE("av_time_diff <0  睡眠:%s，丢包:%f ，剩余包: %d", "睡眠", av_time_diff, frames.queueSize());
            continue;
        } else {
            //完美
        }
...
void VideoChannel::release() {
    LOGE("av_time_diff release 队列-1 size :%d", frames.queueSize());
    isPlaying = false;
    stop();
    ...
    LOGE("av_time_diff release 队列-2 size :%d", frames.queueSize());
